...
namespace InfoMallWebService.Repository
{
	public class ContentForMallRepository : IContentForMallRepository
	{
		...
		public async Task<bool> AddContent(ContentForMallDto newContent)
		{
			...
			{
				relativePath = _img.CreateImage(newContent.File, _env);
			}
		...
		public async Task<List<ContentForMallDto>> GetAllContents() =>
			...
			await _ctx.ContentsForMall.Include(c => c.Category).Select(c => new ContentForMallDto {
		...
		public async Task<ContentForMallDto> GetContentByID(int id, bool didViewContent)
		{
			if (ContentExists(id))
			{
				ContentForMall c = await _ctx.ContentsForMall.Where(co => co.ContentId == id).Include(co => co.Category).SingleOrDefaultAsync();
		...
		public async Task<bool> UpdateContentWithID(ContentForMallDto content)
		{
			if (ContentExists(content.ContentId))
			{
				...
				{
					path = _img.EditImage(content.File, content.OldImage, _env);
				}
		...
		public async Task<(bool, string)> DeleteContentWithID(ContentForMallDto content)
		{
			if (ContentExists(content.ContentId))
			{
				...
				if (target != null)
				{
					...
					{
						_img.DeleteImages(content.ImagesToDelete.ToList(), _env);
					}
					try
					{
						_ctx.Remove(target);
